...
- ✅ Loan with multiple interest where we have overdue on a period is still Overdue status after I completed the overdue period. 
...
- ✅ Let's revert and enable close modal on outside click and just prevent modal on outside click ONLY if we are submitting the form or we have dirty changes from the form (Implemented: Added DialogFormContext that forms use to register their dirty/submitting state. Modals now allow outside click to close by default, but prevent it when forms have unsaved changes or are submitting.)
...
- ✅ Let's make the interest Can still be "Mark as Incomplete" so we can edit/re-edit it. Once we do this, please make sure that we update the total received/balance for that loan.
- ✅ Can we make the export data format be more readable. Instead of CSV, let's export by via PDF which is already printable. Then, make each loan very readable and organize to each other that displays/rendered all enabled information upon export. Render the notes on it's original format where we respect the spacing/new lines. (Implemented: PDF export with @react-pdf/renderer. Loans: rich card-per-loan layout with Financial Summary, Investor Breakdown, Interest Periods, Received Payments, Free Lot, and Notes sections. Investors/Transactions: clean table layouts. All with page headers, footers, grand totals, and configurable section selection modal.)
- Create a vector graph of the payment received and abonos paid between investors
- ✅ Cleanup and remove the dropdown content from table and grid items.
